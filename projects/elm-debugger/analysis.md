...
The major conceptual limitation is that there is no possibility to restore the applications situation by restoring the state. The model is always restored by replaying message sends. This is because the idea behind the debugger is that the user input is the relevant and unchangeable part of a running application. The structure of the model may be changed when the source code is changed and therefore the old state of the application does not work with the new source code. The debugger assumes that while the model may change, the application will always be able to process the input that already happened in a correct manner. This is way the situation is restored by replaying user input and not by restoring state.  
...
![counter unpredictable](ressources/counter_unpredictable.png)
